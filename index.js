  ...
  obtainComponentAtts({ attributes }, enabledAttributes) {
    ...
    const componentAtts = Array.from(attributes)
      ...
      .reduce((acc, { nodeName, nodeValue }) => {
        acc[nodeName] = this.props.fill && nodeName === 'fill' && nodeValue !== 'none' ? this.props.fill : nodeValue
        return acc
      }, {});
    Object.assign(componentAtts, styleAtts);

    const idattr = Array.from(attributes).filter( ({nodeName, nodeValue}) => nodeName == "id" );
    if ( idattr.length > 0 ) {      
      if (typeof this.props["eventsMap"] !== "undefined") {
        // has the id property, find the events if there is any
        const em = this.props.eventsMap[idattr[0].nodeValue];
        if (typeof em !== "undefined") {
          Object.assign(componentAtts, em);
        }
      }
    }
